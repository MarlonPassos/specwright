...
export const PLAN_WRITE_PROTOCOL = [
  ...
  '2. Antes do PRIMEIRO bundle da sessão, leia o contrato:',
  '   `specs project bundle-schema --json`. Ele traz a raiz, o catálogo de',
  '   operações, o `PlannedChangeSpec`, as regras e um exemplo. Não deduza o',
  '   formato por tentativa e erro contra `apply`.',
  '3. Para mudar estrutura, monte um bundle JSON com `expectRevision` igual ao',
  '   `revision` que você leu e rode `specs project apply --dry-run --json`.',
  '   Você NUNCA escolhe um `CH-NNN`: declare `ref: "$nome"` no `addChange` e',
  '   cite `"$nome"` em `dependsOn` e `milestones[].changes` do mesmo bundle;',
  '   os IDs reais voltam em `idMap`.',
  '4. Para materializar documentos, rode',
  '   `specs project generate --milestone <id> --dry-run --json`.',
  '5. Mostre ao usuário o que mudaria e o impacto que o comando devolveu.',
  '6. Faça UMA pergunta direta de sim ou não e espere a confirmação em uma',
  '   mensagem separada. A confirmação vale só para o escopo mostrado.',
  '7. Depois do sim, rode o mesmo comando sem `--dry-run`.',
  '8. Se vier `plan_revision_conflict`, recarregue o estado e repita do passo 1.',
  '9. Se vier `planned_change_modified`, NÃO use `--force` por conta própria:',
  '   mostre o conflito e pergunte.',
  '10. Feche com `specs project validate --strict --json` e reporte o resultado.',
].join('\n');
